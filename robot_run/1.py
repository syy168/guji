...
def print_pose(label: str, pose: Pose, logger) -> None:
    logger.info(
...
class ArucoRelativeTeachNode(Node):
    ...
    def _detect_marker_pose_once(self) -> Optional[Pose]:
        ...
        rvec = np.asarray(rvec[0, 0, :], dtype=np.float64).reshape(3, 1)
        tvec = np.asarray(tvec[0, 0, :], dtype=np.float64).reshape(3, 1)
