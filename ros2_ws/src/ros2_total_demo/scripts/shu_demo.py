...
class CatchNode(Node):
    def __init__(self):
        ...
        #左臂过渡位姿
        self.left_pose = config['LEFT_POSE']

        self.right_middle = config['RIGHT_MIDDLE']
        self.left_middle = config['LEFT_MIDDLE']
        ...
        #左臂发布一个控制机械臂movej运动的消息
        self.left_pub_joint = self.create_publisher(Movej, '/left_arm_controller/rm_driver/movej_cmd', 10)

        #左臂发布一个控制机械臂movej_p运动的消息
        self.left_pub_pose = self.create_publisher(Movejp, '/left_arm_controller/rm_driver/movej_p_cmd', 10)

        #左臂发布一个请求机械臂当前状态的消息
        self.left_pub_get_pose = self.create_publisher(Empty, '/left_arm_controller/rm_driver/get_current_arm_state_cmd', 10)

        #左臂订阅机械臂movej_p运动规划是否成功
        self.left_movej_p_plan_result_sub = self.create_subscription(Bool,'/left_arm_controller/rm_driver/movej_p_result', self.left_movej_p_callback,10)
    ...
    #left_arm movej
    def left_movej(self,joint=[],speed=10,block=True,trajectory_connect=0,dof=6):
        movej=Movej()
        movej.joint=joint
        movej.speed=speed
        movej.block=block
        movej.trajectory_connect=trajectory_connect #0 代表立即规划，1 代表和下一条轨迹一起规划，当为 1 时，轨迹不会立即执行
        movej.dof=dof
        self.left_pub_joint.publish(movej)
    ...
    # left_arm movej_p
    def left_movej_p(self,pose=[],speed=10,block=True,trajectory_connect=0):
        movejp = Movejp()
        movejp.pose.position.x = pose[0]
        movejp.pose.position.y = pose[1]
        movejp.pose.position.z = pose[2]
        ...
        movejp.trajectory_connect=trajectory_connect #0 代表立即规划，1 代表和下一条轨迹一起规划，当为 1 时，轨迹不会立即执行
        movejp.block = block # 是否阻塞
        self.left_pub_pose.publish(movejp)
    ...
    # movej_p运动规划结果回调函数   
    def left_movej_p_callback(self,msg):
        if msg.data:
            self.run_state = True
            print("*******Plan State OK")
        else:
    ...
    def worker_function(self):
        ...
        # 1.agv到达目标点
        #self.agv_send_goal(self.agv_target_name) 
        ...
        self.set_lift_height(300,50)

        # 4.机械臂到达准备抓取位置
        self.movej_p(self.ready_pose,self.speed)
        #self.is_arrive()
        
        self.left_movej_p(self.left_pose,self.speed)
        self.is_arrive()

        self.movej_p(self.right_middle,self.speed)
        self.left_movej_p(self.left_middle,self.speed)
        self.is_arrive()

        self.set_lift_height(600,50)
