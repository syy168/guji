...
def generate_launch_description():
    ...
    # 机器人模型名称
    robot_name_in_model = 'dual_rm_65b_rmg24_gripper_description'
    # 功能包路径
    pkg_share = FindPackageShare(package=package_name).find(package_name) 
    # urdf模型路径
    urdf_model_path = os.path.join(pkg_share, f'config/dual_rm_65b_rmg24_gripper_gazebo.urdf.xacro')
    # world仿真世界路径
    ...
    # 加载左手夹爪控制器
    load_left_rmg24_gripper_controller = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'start',
    ...
    # 加载右手夹爪控制器
    load_right_rmg24_gripper_controller = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'start',
    ...
    close_evt5 = RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=load_joint_state_controller,
                on_exit=[load_left_rmg24_gripper_controller]
            )
    )
    close_evt6 = RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=load_joint_state_controller,
                on_exit=[load_right_rmg24_gripper_controller]
            )
    )
